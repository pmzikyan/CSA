 ...
 *	Wordle.java
 *
 *	Wordle is a game where the user has 6 tries to find a randomly
 * 	selected word. After every try the letters of the word get
 * 	highlighted in different colors. Gray means the letter is not in the
 * 	word at all. Yellow means that the letter is in the word but in a 
 * 	different spot. Green means that that exact letter is in the spot
 * 	it's supposed to be.
 *
 *	@author	Scott DeRuiter and David Greenstein and Petros Mzikyan
 *	@version	1.0
 *	@since		10/7/2024
 */ 
public class Wordle
{ 
	...
	public String openFileAndChooseWord(String inFileName, String testWord)
	{
		...
		int goalWordsAmount = 0;
		FileUtils goalWords = new FileUtils();
		Scanner goalsFile = goalWords.openToRead(WORDS5);
